...
int16 fragment(IP_DGRAM **datagram, uint16 mtu)
{
	...
	do
	{
		...
		if (first_flg)
		{
			orig = (*datagram)->options;
			count = (*datagram)->opt_length;
			first_flg = FALSE;
		} else
		{
			orig = remain;
...
int16 reassembly(IP_DGRAM **datagram, int16 protocol)
{
	...
	src = (*datagram)->hdr.ip_src;
	ident = (*datagram)->hdr.ident;
	...
	if (walk->act_space < needed)
	{
		...
		if (n_data == NULL || n_bit == NULL)
		{
			...
			KRfree(walk->dgram->options);
			KRfree(walk->dgram);
			KRfree(walk);
			return FALSE;
		}
		walk->dgram->pkt_data = n_data;
		walk->blk_bits = n_bit;
	...
	for (count = 0; count <= (*datagram)->pkt_length / 8; count++)
	{
		...
		byte = (uint8 *) walk->blk_bits + bit / 8;
		*byte |= (1 << (bit % 8));
	}
	...
	*datagram = NULL;
	...
	if (walk->ttl_data)
	{
		...
		IP_discard(*datagram, TRUE);
		*datagram = dgram;
		KRfree(walk->blk_bits);

		if (previous)
			previous->next = walk->next;
		else
			ip[protocol].defrag = walk->next;
		KRfree(walk);
