...
int16   cdecl  my_TCP_close (int16 connec, int16 mode);
...
int16  cdecl  my_TCP_close (connec, mode)

int16  connec, mode;

{
   CONNEC  *conn, *work, **previous;
   ...
   switch (conn->state) {
      ...
      case TESTABLISH :
      case TCLOSE_WAIT :
        conn->send.count++;   conn->send.next++;
      ...
      case TTIME_WAIT :
        rel_flag (& conn->sema);
        while (conn->state != TCLOSED) {
             _appl_yield();
             if (TIMER_elapsed (timer) > time_out) {
                  abort (conn);
                  close_self (conn, R_TIMEOUT);
                  break;
                }
           }
        wait_flag (& conn->sema);
   ...
   flush_queue (& conn->send.queue);
   flush_queue (& conn->recve.queue);

/* finish pending, reseq */
