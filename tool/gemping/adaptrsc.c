...
static _WORD __CDECL check_button(PARMBLK *parmblock);
static _WORD __CDECL radio_button(PARMBLK *parmblock);
static _WORD __CDECL group(PARMBLK *parmblock);
static _WORD __CDECL title(PARMBLK *parmblock);
...
static _WORD __CDECL check_button(PARMBLK *parmblock)
{
	...
	userdef_text(parmblock->pb_x + gl_hchar + gl_wchar, parmblock->pb_y, string);
...
static _WORD __CDECL group(PARMBLK *parmblock)
{
	...
	xy[8] = obj[2];
	xy[9] = xy[1];
	xy[10] = (short) (xy[0] + strlen(string) * gl_wchar);
	xy[11] = xy[1];
