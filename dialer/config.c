...
int port_lock = FALSE;
int has_LAN;
char const ip_out[] = "%d.%d.%d.%d";

static _WORD const conf_box[] = { CM_BOX, CD_BOX, CP_BOX, CA_BOX, CT_BOX, CN_BOX, CG_BOX };
_WORD const edit[] = { CM_INIT, CD_TOUT, CP_IP, CA_USER, CT_NUM, CN_ED1, 0 };
static _WORD const environ_objs[] = { CN_ED1, CN_ED2, CN_ED3, CN_ED4, CN_ED5, CN_ED6, CN_ED7 };

static char const script_ext[] = "\\*.SCR";
static char const fee_ext[] = "\\*.FEE";
static char const read_problem[] = "[1][ |   Cannot find/read file !   ][ Cancel ]";
static char const write_problem[] = "[1][ |   Cannot write file !   ][ Cancel ]";
static char const alert_form[] = "[1][ |  Wrong format in script variable  |     " "\'%s\'|  Line ignored !][ Oops ]";
static char const path_prob[] = "[1][ |  Fee File can only be found in" "  | |  script directory !][ Proceed | Cancel ]";
static char const len_prob[] = "[1][ |  Fee File name length must not" "  | |  exceed 12 characters !][ Cancel ]";
static char const ip_format[] = "%-3d%-3d%-3d%-d";

char script_path[256];

char modem_init[17] = "ATZ";
char modem_dial[9] = "ATDT";
char modem_hangup[9] = "+++,ATH";
int connect_timeout = 60;
int redial_delay = 20;

static _WORD const o_conn[3] = { CM_CON1, CM_CON2, CM_CON3 };
static _WORD const o_abrt[3] = { CM_ABO1, CM_ABO2, CM_ABO3 };
static _WORD const o_redl[3] = { CM_RED1, CM_RED2, CM_RED3 };
char s_conn[3][13] = { "CONNECT", "", "" };
char s_abrt[3][13] = { "NO DIALTONE", "", "" };
char s_redl[3][13] = { "BUSY", "NO CARRIER", "" };

static int script_which = 0;
int script_length = 0;
...
char fullname[33];
char fqdn[43];
static int act_dns = 0;
int dns_num = 0;

static int act_phone = 0;
int phone_num = 0;
char phone[8][17];
char fee_file[13];
...
char **environ_base;
static char *environ_used;
int environ_number;
static int environ_offset;
...
int run_tools = FALSE;
int compuserve = FALSE;
int resident = FALSE;
int debugging = FALSE;


static int alert_mode; 
...
int init_config_stuff(void)
{
	...
	tree[CD_BOX].ob_flags |= OF_HIDETREE;
	parse_tree(tree, CD_BOX, 0);

	tree[CP_BOX].ob_x = tree[CM_BOX].ob_x, tree[CP_BOX].ob_y = tree[CM_BOX].ob_y;
	tree[CP_BOX].ob_flags |= OF_HIDETREE;
	parse_tree(tree, CP_BOX, 0);

	tree[CA_BOX].ob_x = tree[CM_BOX].ob_x, tree[CA_BOX].ob_y = tree[CM_BOX].ob_y;
	tree[CA_BOX].ob_flags |= OF_HIDETREE;
	parse_tree(tree, CA_BOX, 0);

	tree[CT_BOX].ob_x = tree[CM_BOX].ob_x, tree[CT_BOX].ob_y = tree[CM_BOX].ob_y;
	tree[CT_BOX].ob_flags |= OF_HIDETREE;
	parse_tree(tree, CT_BOX, 0);

	tree[CN_BOX].ob_x = tree[CM_BOX].ob_x, tree[CN_BOX].ob_y = tree[CM_BOX].ob_y;
	tree[CN_BOX].ob_flags |= OF_HIDETREE;
	parse_tree(tree, CN_BOX, 0);

	tree[CG_BOX].ob_x = tree[CM_BOX].ob_x, tree[CG_BOX].ob_y = tree[CM_BOX].ob_y;
	tree[CG_BOX].ob_flags |= OF_HIDETREE;
	parse_tree(tree, CG_BOX, 0);
...
static void set_tedinfo_text(_WORD rsc_tree, _WORD object, const char *text)
{
	change_tedinfo(rsc_tree, object, -1, TE_PTEXT, text, -1);
}


static void set_tedinfo_number(_WORD rsc_tree, _WORD object, int number)
{
	char temp[32];
...
void fill_in_config_box(void)
{
	...
	char temp[42];
	char *ptr;
	static char const frmt[] = "%ld.%03ld";
	int count;
	...
	sprintf(temp, ip_format, (int) ip_address[0], (int) ip_address[1], (int) ip_address[2], (int) ip_address[3]);
	...
	if (protocol != 0)
	{
		tree[CP_PID].ob_flags |= OF_EDITABLE;
		ENABLE(CP_PID);
		tree[CP_PPASS].ob_flags |= OF_EDITABLE;
		ENABLE(CP_PPTXT);
		ENABLE(CP_FPPW);
		ENABLE(CP_PPPW);
		ENABLE(CP_TPPW);
	...
	if (dns_num)
	{
		sprintf(temp, ip_format, ip_dns[act_dns][0], ip_dns[act_dns][1], ip_dns[act_dns][2], ip_dns[act_dns][3]);
	}
	...
	*(strchr(temp, '.') + digits + 1) = '\0';
	set_tedinfo_text(CONF, CI_TC, temp);
	sprintf(temp, frmt, ISP_c_sent / 1000, ISP_c_sent % 1000);
	*(strchr(temp, '.') + digits + 1) = '\0';
	set_tedinfo_text(CONF, CI_SC, temp);
	sprintf(temp, frmt, ISP_c_recvd / 1000, ISP_c_recvd % 1000);
	*(strchr(temp, '.') + digits + 1) = '\0';
	set_tedinfo_text(CONF, CI_RC, temp);

	environ_offset = 0;
	set_tedinfo_text(CONF, CN_OFF, "  0 ");
	for (count = 0; count < 7; count++)
	{
		set_tedinfo_text(CONF, environ_objs[count], count < environ_number ? environ_base[count] : "");
	}
...
static void ted_ip_addr(unsigned char *ip, const char *buffer)
{
	char byte[4];
	char temp[15];
...
static void read_cost(char *buffer, long *ISP_unit, long *ISP_cost)
{
	char *work;
	char *dot;
	...
	*ISP_cost = atol(work) * ((digits == 0) ? 1000 : ((digits == 1) ? 100 : ((digits == 2) ? 10 : 1)));
}


static void get_tedinfo_text(_WORD rsc_tree, _WORD object, char *text)
{
	OBJECT *tree;
	TEDINFO *ted;
...
static void get_tedinfo_number(_WORD rsc_tree, _WORD object, int *number)
{
	OBJECT *tree;
...
static void read_out_config_box(void)
{
	OBJECT *tree;
	char temp[32];
	static char const frmt[] = "%s, %s";
	int count;
	...
	get_tedinfo_text(CONF, CP_IP, temp);
	ted_ip_addr(ip_address, temp);
	get_tedinfo_number(CONF, CP_MTU, &port_mtu);
	get_tedinfo_text(CONF, CP_PID, pap_id);
	get_tedinfo_text(CONF, CP_PPASS, pap_passwd);
	protocol = (tree[CP_SLIP].ob_state & OS_SELECTED) ? 0 : 1;
	papp_flag = (tree[CP_FPPW].ob_state & OS_SELECTED) ? 0 : 1;
	port_flags = protocol;
	port_flags |= (tree[CP_VJHC].ob_state & OS_SELECTED) ? 2 : 0;
	port_flags |= (tree[CP_LAN].ob_state & OS_SELECTED) ? 4 : 0;
	...
	for (count = 0; count < dns_num; count++)
		...
		{
			--dns_num;
			ip_dns[count][0] = ip_dns[dns_num][0];
			ip_dns[count][1] = ip_dns[dns_num][1];
			ip_dns[count][2] = ip_dns[dns_num][2];
			ip_dns[count][3] = ip_dns[dns_num][3];
		}
	port_flags |= (tree[CA_GETNS].ob_state & OS_SELECTED) ? 8 : 0;
	...
	def_route = (tree[CG_DEFRT].ob_state & OS_SELECTED) ? TRUE : FALSE;
	masquerade = (tree[CG_MASQU].ob_state & OS_SELECTED) ? TRUE : FALSE;
	run_tools = (tree[CG_LOGIN].ob_state & OS_SELECTED) ? TRUE : FALSE;
	compuserve = (tree[CG_CIX].ob_state & OS_SELECTED) ? TRUE : FALSE;
	resident = (tree[CG_RESI].ob_state & OS_SELECTED) ? TRUE : FALSE;
	debugging = (tree[CG_DEBUG].ob_state & OS_SELECTED) ? TRUE : FALSE;
}
...
static char get_char(int handle, int *eof)
{
	static char buffer[256];
	static int level;
	static int eof_level;
...
static int compare(char *buffer, int *flag, const char *fix_strg)
{
	char *b_ptr;
	const char *f_ptr;
	char compare_char;
...
static int read_boolean(const char *buffer)
{
...
static int read_environment(char *buffer)
{
	char name[32];
	char value[64];
	char *work;
	char **array = environ_base;
	int length;
	int count;
...
static void load_config_part(int handle, int *eof)
{
	int i_conn = 0;
	int i_redl = 0;
	int i_abrt = 0;
	...
	do
	{
		...
		*work = '\0';
		...
		if (compare(buffer, &flag, "INIT"))
			strncpy(modem_init, buffer, 16);
		if (compare(buffer, &flag, "PREFIX"))
			strncpy(modem_dial, buffer, 8);
		if (compare(buffer, &flag, "HANGUP"))
			strncpy(modem_hangup, buffer, 8);
		if (compare(buffer, &flag, "CONNECT_WAIT"))
			connect_timeout = atoi(buffer);
		if (compare(buffer, &flag, "REDIAL_DELAY"))
			redial_delay = atoi(buffer);

		if (compare(buffer, &flag, "SUCCESS"))
		{
			if (i_conn < 3)
				strncpy(s_conn[i_conn++], buffer, 12);
		}
		if (compare(buffer, &flag, "FAILURE"))
		{
			if (i_redl < 3)
				strncpy(s_redl[i_redl++], buffer, 12);
		}
		if (compare(buffer, &flag, "ABORT"))
		{
			if (i_abrt < 3)
				strncpy(s_abrt[i_abrt++], buffer, 12);
		}

		if (compare(buffer, &flag, "CLIENT_IP"))
			load_ip_addr(ip_address, buffer);
		if (compare(buffer, &flag, "MTU"))
			port_mtu = atoi(buffer);
		if (compare(buffer, &flag, "PAP_ID"))
			strncpy(pap_id, buffer, 30);
		if (compare(buffer, &flag, "PAP_PASSWORD"))
			strncpy(pap_passwd, buffer, 30);
		if (compare(buffer, &flag, "PAP_PROMPT"))
			papp_flag = read_boolean(buffer);
		if (compare(buffer, &flag, "PROTOCOL"))
			protocol = atoi(buffer);
		if (compare(buffer, &flag, "VJHC"))
			port_flags |= read_boolean(buffer) ? 2 : 0;
		if (compare(buffer, &flag, "LAN_PORT"))
			port_flags |= read_boolean(buffer) ? 4 : 0;

		if (compare(buffer, &flag, "USERNAME"))
			strncpy(username, buffer, 16);
		if (compare(buffer, &flag, "FULLNAME"))
			strncpy(fullname, buffer, 32);
		if (compare(buffer, &flag, "HOSTNAME"))
			strncpy(fqdn, buffer, 42);

		if (compare(buffer, &flag, "NAMESERVER"))
			if (dns_num < 4)
				load_ip_addr(ip_dns[dns_num++], buffer);
		if (compare(buffer, &flag, "FETCH_DNS"))
			port_flags |= read_boolean(buffer) ? 8 : 0;

		if (compare(buffer, &flag, "PHONE_NUMBER"))
			if (phone_num < 8)
				strncpy(phone[phone_num++], buffer, 16);

		if (compare(buffer, &flag, "FEE_FILE"))
			strncpy(fee_file, buffer, 12);
		if (compare(buffer, &flag, "ISP_TIME"))
			read_cost(buffer, &ISP_u_time, &ISP_c_time);
		if (compare(buffer, &flag, "ISP_SENT"))
			read_cost(buffer, &ISP_u_sent, &ISP_c_sent);
		if (compare(buffer, &flag, "ISP_RECVD"))
			read_cost(buffer, &ISP_u_recvd, &ISP_c_recvd);

		if (compare(buffer, &flag, "SERIALPORT"))
			strncpy(port_txt, buffer, 10);
		if (compare(buffer, &flag, "DEF_ROUTE"))
			def_route = read_boolean(buffer);
		if (compare(buffer, &flag, "MASQUERADE"))
			masquerade = read_boolean(buffer);
		if (compare(buffer, &flag, "EXEC_BATCH"))
			run_tools = read_boolean(buffer);
		if (compare(buffer, &flag, "CIX_LOGIN"))
			compuserve = read_boolean(buffer);
		if (compare(buffer, &flag, "RESIDENT"))
			resident = read_boolean(buffer);
		if (compare(buffer, &flag, "DEBUGGING"))
			debugging = read_boolean(buffer);

		if (compare(buffer, &flag, "FIND_TIMEOUT"))
			script_timeout = atoi(buffer);

		if (compare(buffer, &flag, "DIAL_SCRIPT"))
			break;
		if (flag == FALSE)
			read_environment(buffer);
	} while (!*eof);
...
static int load_script_part(int handle, int *eof)
{
	int index;
	int check;
	int flag;
	static char const alert[] = "[1][ |  Dial script : Wrong sequence   | |  of WAIT/REPT/FIND/RESP !][ Oops ]";
	char buffer[256];
	char *work;
	char *scr_ptr;
	char *script_buffer;
	...
	if (!*eof)
	{
		...
		do
		{
			...
			*scr_ptr = '\0';
			if (compare(buffer, &flag, "WAIT"))
				*scr_ptr = 'W';
			if (compare(buffer, &flag, "REPT"))
				*scr_ptr = 'P';
			if (compare(buffer, &flag, "FIND"))
				*scr_ptr = 'F';
			if (compare(buffer, &flag, "RESP"))
				*scr_ptr = 'R';
			if (*scr_ptr)
			{
	...
	script[index][0][0] = script[index][1][0] = script[index][2][0] = script[index][3][0] = '\0';
	script_length = index;
...
static int choose_dial_script(void)
{
	_WORD button;
	int handle;
	int eof = 1;
	...
	if (strrchr(path, '\\'))
	{
		strcpy(strrchr(path, '\\'), script_ext);
		strncpy(file, strrchr(script_path, '\\') + 1, 32);
		file[31] = '\0';
	} else
	{
		strcat(path, script_ext);
		file[0] = '\0';
	}
...
static void write_text_line(int handle, const char *desc, const char *text)
{
	char buffer[128];
...
static void write_number_line(int handle, const char *desc, int number)
{
	char buffer[128];
...
static void save_config_part(int handle)
{
	int count;
	char text[33];
	static char const cost[] = "%ld, %ld.%03ld";
	...
	Fwrite(handle, 27L, "#\r\n#   Port Parameters\r\n#\r\n");
	sprintf(text, ip_out, (int) ip_address[0], (int) ip_address[1], (int) ip_address[2], (int) ip_address[3]);
	...
	for (count = 0; count < dns_num; count++)
	{
		if (ip_dns[count][0] != 0)
		{
			sprintf(text, ip_out, ip_dns[count][0], ip_dns[count][1], ip_dns[count][2], ip_dns[count][3]);
...
static void save_script_part(int handle)
{
	int count;
...
static void save_dial_script(void)
{
	_WORD button;
	int handle;
	long error;
...
static void get_fee_file(void)
{
	...
	{
		if (form_alert(1, path_prob) == 2)
			return;
	}
	if (strlen(file) > 12)
	{
		form_alert(1, len_prob);
		return;
	}
...
int conf_click(_WORD object)
{
	...
	_WORD index;
	_WORD last_shown;
	static char const alert[] = "[3][ |  Not possible while  | |  port enabled !][ Hmmm ]";
	...
	case CC_M_NAM:
		...
		if (last_shown != --conf_shown)
		{
			...
			if (tree[CP_SLIP].ob_state & OS_SELECTED)
			{
				tree[CP_PID].ob_flags &= ~OF_EDITABLE;
				tree[CP_PPASS].ob_flags &= ~OF_EDITABLE;
			}
	...
	case CP_SLIP:
		...
		tree[CP_PID].ob_flags &= ~OF_EDITABLE;
		tree[CP_PPASS].ob_flags &= ~OF_EDITABLE;
		change_flags(CONF, CP_PID, 1, 0, OS_DISABLED);
		change_flags(CONF, CP_PPTXT, 1, 0, OS_DISABLED);
		change_flags(CONF, CP_FPPW, 1, 0, OS_DISABLED);
		change_flags(CONF, CP_PPPW, 1, 0, OS_DISABLED);
		change_flags(CONF, CP_TPPW, 1, 0, OS_DISABLED);
		DISABLE(CA_GETNS);
		DISABLE(CA_NSTXT);
	...
	case CP_PPP:
		tree[CP_PID].ob_flags |= OF_EDITABLE;
		tree[CP_PPASS].ob_flags |= OF_EDITABLE;
		change_flags(CONF, CP_PID, 0, 0, OS_DISABLED);
		change_flags(CONF, CP_PPTXT, 0, 0, OS_DISABLED);
		change_flags(CONF, CP_FPPW, 0, 0, OS_DISABLED);
		change_flags(CONF, CP_PPPW, 0, 0, OS_DISABLED);
		change_flags(CONF, CP_TPPW, 0, 0, OS_DISABLED);
		ENABLE(CA_GETNS);
		ENABLE(CA_NSTXT);
		break;
	case CA_PREV:
	...
	case CA_ADD:
		...
		if (count < 4)
		{
			...
			ip_dns[act_dns][0] = ip_dns[act_dns][1] = ip_dns[act_dns][2] = ip_dns[act_dns][3] = 0;
	...
	case CT_NEXT:
		...
		{
			if (--act_phone < 0)
				act_phone = phone_num - 1;
		} else
		{
			if (++act_phone >= phone_num)
				act_phone = 0;
		}
	...
	case CT_ADD:
		...
		{
			change_flags(CONF, CT_ADD, 1, 0, OS_DISABLED);
		}
		interupt_editing(CONF, END, CT_NUM);
		break;
	case CT_FEE:
...
int load_dial_script(const char *filename)
{
	int handle;
	int eof = 1;
	int ret_code = 1;
...
uint32 load_ip_addr(unsigned char *ip, const char *str)
{
	...
	if (ip_1 >= 0 && ip_1 < 256 && ip_2 >= 0 && ip_2 < 256 && ip_3 >= 0 && ip_3 < 256 && ip_4 >= 0 && ip_4 < 256)
	{
		ip[0] = (unsigned char) ip_1;
		ip[1] = (unsigned char) ip_2;
		ip[2] = (unsigned char) ip_3;
		ip[3] = (unsigned char) ip_4;
	} else
	{
		return 0;
	}
