...
mfp1		EQU		0x00fffa01
mfp2		EQU		0x00fffa81
sccctl_a	EQU		0xffff8c81
sccdat_a	EQU		0xffff8c83
sccctl_b	EQU		0xffff8c85
sccdat_b	EQU		0xffff8c87

/* *** Bco* - Routinen: */
bcon_m1		EQU		0x0522.w
bcon_midi	EQU		0x052A.w
...
.dtr_off1:
;		move.w		#0x10,-(sp)			; DTR-Bit oderieren
;		move.w		#0x1e,-(sp)			; Opcode fr Ongibit (XBIOS 30)
		move.l		#0x001e0010,-(sp)	;
		trap		#14					; XBIOS-Trap
...
.dtr_off2:
		...
		move.w		#0x26,-(sp)			; Opcode fr Supexec (XBIOS 38)
		trap		#14					; XBIOS-Trap
...
.low_it2:
		...
		ori.w		#0x700,sr			; Interrupts sperren
		
		movea.l		dev(pc),a0			; Iorec Struktur holen
		move.b		0x1d(a0),d0			; WR5 Schatten nach d0
		andi.b		#0x7f,d0			; DTR aus
		move.b		d0,0x1d(a0)			; WR5 Schatten zurckschreiben
		
		lea			sccctl_b,a0			; Adresse des SCC-Control-Registers
		move.b		#0x05,(a0)			; Write-Register 5 ausw„hlen
...
.dtr_on1:
;		move.w		#0xffe7,-(sp)		; DTR- und RTS-Bit undieren
;		move.w		#0x1d,-(sp)			; Opcode fr Offgibit (XBIOS 29)
		move.l		#0x001dffe7,-(sp)	;
		trap		#14					; XBIOS-Trap
		addq.l		#4,sp				; Stack restaurieren
		rts
...
SetMapM1:
		...
		clr.w		-(sp)				; dev: RS232
		move.w		#0xE,-(sp)			; Opcode fr Iorec (XBIOS 14)
		trap		#14					; XBIOS-Trap
		addq.l		#4,sp
