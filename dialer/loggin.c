...
static UNI_TIME begin;
static UNI_TIME end;
static UNI_TIME easter;
static UNI_TIME advent;
static int duration[17];

static int tariff_next;
static long fee_unit;
static int max_days[] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

static char const month[][4] = {
	"Jan", "Feb", "Mar", "Apr", "May", "Jun",
	"Jul", "Aug", "Sep", "Oct", "Nov", "Dec"
};
static char const weekday[][4] = { "Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat" };

static char tariff_name[17];
static char log_buffer[256];
static char *tariff_time[200];
...
static int find_next_cr(char **walk)
{
	int count;
...
static void convert_time(const char *string, UNI_TIME *this_time)
{
	int count;
...
static long calc_factor(UNI_TIME *now)
{
	long factor;
...
static void add_days(UNI_TIME *now, int number)
{
	int temp;
...
static void calc_festivals(UNI_TIME *now)
{
	long temp;
	...
	temp += ((easter.tm_year % 4) * 2 + (easter.tm_year % 7) * 4 + temp * 6 + 5) % 7 + 21;
	easter.tm_mday = 1;
	easter.tm_mon = 2;
...
static int get_time_info(char **walk, UNI_TIME *this_time, int end_flag)
{
	while (**walk == ' ')
		(*walk)++;
...
static int is_later(UNI_TIME *now, UNI_TIME *reference)
{
...
static long test_interval(UNI_TIME *now, int index)
{
	UNI_TIME lower;
	UNI_TIME upper;
	UNI_TIME temp;
	...
	more_time = (reference->tm_hour - now->tm_hour) * 3600L + (reference->tm_min - now->tm_min) * 60L - now->tm_sec;

	return more_time;
}


static int read_offset(int index, int *offset, int lower, int upper)
{
	int neg = 0;
	char *walk;
	char *test;
...
static int find_tariff(UNI_TIME *now, long *more_time)
{
	UNI_TIME temp;
	int count;
	int offset;
	...
	for (count = 0; count < tariff_next; count++)
	{
		...
		case 'A':
			if (prio < 2)
			{
				...
				if (temp.tm_year == now->tm_year && temp.tm_mon == now->tm_mon && temp.tm_mday == now->tm_mday)
					if ((*more_time = test_interval(now, count)) > 0)
					{
						prio = 2;
						tariff = tariff_time[count][0];
...
static void add_duration(UNI_TIME *now, long duration)
{
	long temp;

	temp = now->tm_sec + duration;
	now->tm_sec = (int) (temp % 60);
	temp = now->tm_min + temp / 60;
	now->tm_min = (int) (temp % 60);
	temp = now->tm_hour + temp / 60;
	now->tm_hour = (int) (temp % 24);

	add_days(now, (int) (temp / 24));
}


static long calc_charge(long dur_total)
{
	int tariff;
	long charge = 0;
	long more_time = 0;
	...
	do
	{
		tariff = find_tariff(&begin, &more_time);
		if (tariff == -1)
			return -1;
...
void write_extra_line(void)
{
	...
	temp.tm_hour = (int) (calc / 3600);
	calc %= 3600;
	temp.tm_min = (int) (calc / 60);
	calc %= 60;
	...
	sprintf(extra_line, "  IP traffic %ld Kb sent, %ld Kb received", CEIL(sent, 1024), CEIL(received, 1024));
	...
	do
	{
		...
		case '+':
			...
			case 'e':
				...
				fee_unit = atol(&line[2]) * (digits == 0 ? 1000 : (digits == 1 ? 100 : (digits == 2 ? 10 : 1)));
				break;
			case 'u':
