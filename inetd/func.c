...
ISM_PARA parameter;
ISM_INTERN *ism_data;
char const version[] = "00.01";
int num_modules = 0;
OBJECT **my_tree_index;
...
static void remove_overlay(int which)
{
	ISM_INTERN *ism;
...
static void load_overlay(int which)
{
	ISM_INTERN *ism;
	BASPAG *basepage;
	long length;
	...
	ism = &ism_data[which];
	basepage = (BASPAG *) Pexec(3, ism->file, "", "");
	if ((long) basepage < 0)
	{
		ism->basepage = (BASPAG *) -1L;
		return;
	...
	Mshrink(basepage, length);

	ism->ism_init = (ISM_SPECS *(*)(ISM_PARA *module_data)) (basepage->p_tbase);
	ism->ism_term = (void (*)(ISM_PARA *module_data)) ((long) basepage->p_tbase + ism->ism_dterm);
	ism->ism_user = (void (*)(ISM_PARA *module_data)) ((long) basepage->p_tbase + ism->ism_duser);
	ism->ism_server = (void (*)(ISM_PARA *module_data)) ((long) basepage->p_tbase + ism->ism_dserver);
...
int init_modules(void)
{
	...
	{
		tree[ST_SLIDE].ob_flags |= OF_HIDETREE;
		for (count = 0; count < 4; count++)
			if (count >= num_modules)
			{
				tree[mdle_box[count]].ob_flags |= OF_HIDETREE;
				tree[click_box[count]].ob_flags |= OF_HIDETREE;
			}
	...
	while (!error)
	{
		...
		ism->rsc_num = values->ism_num_trees;
		index += ism->rsc_num;
...
void insert_modules(int draw_it)
{
	ISM_INTERN *ism;
	OBJECT *tree;
	int count;
