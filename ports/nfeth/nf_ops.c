...
static long __asm__ cdecl _nf_get_id(const char *feature_name)
{
	dc.w NATFEAT_ID
	rts
}
...
static long __asm__ cdecl _nf_call(long id, ...)
{
	dc.w NATFEAT_CALL
	rts
}
...
static long cdecl _nf_get_id(const char *feature_name)
{
	UNUSED(feature_name);
...
static long cdecl _nf_call(long id, ...)
{
	UNUSED(id);
