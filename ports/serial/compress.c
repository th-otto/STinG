...
static int send_uncomp(IP_DGRAM *dgram, VJHC *vjhc, CSTATE *state)
{
...
int vjhc_compress(IP_DGRAM *dgram, VJHC *vjhc)
{
	CSTATE *state;
	CSTATE *walk;
	IP_HDR *ip;
		...
		*(uint32 *) tcp != *(uint32 *) & state->cs_tcp)
	{
		...
		{
			walk->next = state->next;
			state->next = vjhc->last_cstate->next;
			vjhc->last_cstate->next = state;
		}
	}

	old = (int16 *) ip;
	new = (int16 *) & state->cs_ip;
	...
	case NEW_S | NEW_A:
		if (diff_ == diffA && diff_ == (uint32)found) /* ??? == found? */
		{
			vjhc->header[0] = SPECIAL_I;
			work = &vjhc->header[4];
		}
		break;

	case NEW_S:
		if (diff_ == (uint32)found) /* ??? == found? */
		{
			vjhc->header[0] = SPECIAL_D;
			work = &vjhc->header[4];
		}
		break;
	}
...
int vjhc_uncompress(uint8 *buffer, int16 buff_len, int16 type, VJHC *vjhc, IP_DGRAM **dgram)
{
	IP_HDR *ip;
	TCP_HDR *tcp;
	CSTATE *state;
